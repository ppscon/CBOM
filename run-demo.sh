...
trigger_pipeline() {
    ...
    # Trigger the workflow in private demo repo
    gh workflow run "cbom-fips-pipeline-aqua.yml" --ref master --repo ppscon/CBOM-FIPS-Demo
    ...
    LATEST_RUN=$(gh run list --workflow="cbom-fips-pipeline-aqua.yml" --repo ppscon/CBOM-FIPS-Demo --limit 1 --json databaseId,status --jq '.[0] | select(.status == "queued" or .status == "in_progress" or .status == "waiting") | .databaseId')

    if [ -z "$LATEST_RUN" ]; then
        sleep 3
        LATEST_RUN=$(gh run list --workflow="cbom-fips-pipeline-aqua.yml" --repo ppscon/CBOM-FIPS-Demo --limit 1 --json databaseId --jq '.[0].databaseId')
    fi
    ...
    echo "  ${CYAN}https://github.com/ppscon/CBOM-FIPS-Demo/actions/runs/$LATEST_RUN${NC}"
    echo ""
}

watch_pipeline() {
    # Get the latest run from private repo
    LATEST_RUN=$(gh run list --workflow="cbom-fips-pipeline-aqua.yml" --repo ppscon/CBOM-FIPS-Demo --limit 1 --json databaseId --jq '.[0].databaseId')
    ...
    gh run watch "$LATEST_RUN" --repo ppscon/CBOM-FIPS-Demo || true
}

list_runs() {
    print_header "📋 Recent Pipeline Runs"
    gh run list --workflow="cbom-fips-pipeline-aqua.yml" --repo ppscon/CBOM-FIPS-Demo --limit 5
}
